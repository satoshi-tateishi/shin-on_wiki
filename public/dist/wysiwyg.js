...
var DetailsNode = class _DetailsNode extends ElementNode8 {
  ...
  createDOM(_config, _editor) {
    ...
    if (this.__open) {
      ...
      el3.removeAttribute("contenteditable");
    } else {
...
function $unwrapDetailsNode(node) {
  const children = node.getChildren();
  for (const child of children) {
    node.insertBefore(child);
  }
...
function unwrapDetailsNode(context, event) {
  const selection = $getSelection();
  const nodes = selection?.getNodes() || [];
  if (nodes.length !== 1) {
    return false;
...
function registerKeyboardHandling(context) {
  const unregisterBackspace = context.editor.registerCommand(KEY_BACKSPACE_COMMAND, (event) => {
...
function isHardToEscapeNode(node) {
  return $isDecoratorNode(node) || $isImageNode(node) || $isMediaNode(node) || $isDiagramNode(node) || $isTableNode(node) || $isDetailsNode(node);
}
function $getContextNode(event) {
...
var trackedDomNodes = /* @__PURE__ */ new Set();
function setSelectionIndicator(context) {
...
function registerSelectionHandling(context) {
  const unregisterSelectionChange = context.editor.registerCommand(SELECTION_CHANGE_COMMAND, () => {
